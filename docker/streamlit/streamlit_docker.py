...
with st.sidebar:
    ...
    st.title("Navigation")
    page = st.selectbox("Choisissez une page", ["Présentation du Projet", "Interface Utilisateur (APIs)", "Schémas", "MLflow", "Conclusion"])
...
if page == "Présentation du Projet":
    ...
    if api_choice == "Utilisateur":
        ...
        choix_schema = st.radio("Choisissez un schéma :", ["Interaction utilisateur", "Architecture", 'Pipeline'])
        if choix_schema == "Interaction utilisateur":
            ...
            try:
                ...
                st.error("Le fichier SVG n'a pas été trouvé. Assurez-vous qu'il est présent dans le répertoire du script.")
            ...
            2. **Inférence** :
            - La prédiction de classe est effectuée en quelques centaines de milisecondes
            ...
            Les données sont réparties entre un dossier dataset_raw (données brutes) et dataset_clean (données ayant passé le traitement des données (preprocessing)).
            ...
            - Lorsqu'un certain nombre de nouvelles images (au moins l'équivalent de 1% de la totalité du dataset) est ajouté par les utilisateurs, on lance un preprocessing.
            - Lorsqu'une nouvelle classe apparaît, elle ne passe jamais à travers le script de preprocessing, même si déclenché par les évennements ci-dessus, tant qu'elle n'a pas suffisament d'images.
            On considère qu'une classe est complète lorsqu'elle dispose au moins du même nombre d'images que la classe la plus petite de notre dataset. Lorsque la condition est remplie, on lance un preprocessing
            et cette classe se voit intégrée.
            Dès qu'un preprocessing est lancé, cela signifie que les données ont changé de manière suffisante pour déclencher une alerte aux administrateurs et les encourage à ré-entrainer le modèle.
            ...
            Lorsqu'un entraînement est terminé, l'administrateur est notifié et sont enregistrés dans MLFlow.
            ...
            4. **MLFlow** :
            Durant et après un entraînement, MLFlow s'assure du suivi des métriques et de l'enregistrement des modèles ainsi que de la matrice evoquée plus haut.
            Il dispose d'une interface permettant d'afficher pleins d'informations sur les entraînements et de faire des comparaisons entre des modèles.
            Son intêret est aussi d'archiver les modèles.

            5. **Évaluation du modèle** :
            Une fois un entraînement terminé, l'administrateur peut afficher une comparaison entre le modèle en production et celui qui a vient d'être entraîné.
            Pour cela, il dispose des deux métriques les plus importantes, à savoir la validation_accuracy et la validation_loss, mais également d'une liste des 10 pires classes :
            cette liste représente les noms et les scores des classes les moins performantes, permettant d'améliorer la facilité de prise de décision pour changer de modèle.
            ...
            Le script chargé de l'inférence pour les utilisateurs charge en quelques secondes le nouveau modèle, sans interruption de service 
            (seulement une attente si prédiction demandée en même temps que le changement de modèle).

            7. **Monitoring des performances** :
            Grâce à la matrice de confusion enregistré avec chaque modèle, tous les jours, une nouvelle matrice est générée et comparée avec l'originale du modèle.
            Grâce à cela, il est possible de détecter un drift du modèle, c'est à dire qu'il pert en performance sur les classes qu'il connaît à cause de nouvelles images qui peuvent être trop différentes de son entraînement.
            Un rapport est envoyé tous les jours avec les 10 meilleures et pires classes ainsi que les classes qui dérivent (positivement ou négativement, si il y en a).
