...
def send_email(message):
    ...
    login_password = os.getenv("GMAIL_PASSWORD")
...
def preprocessing():
    ...
    try:
        ...
        dataset_clean_csv = os.path.join(volume_path, 'dataset_clean.csv')
        os.remove(dataset_clean_csv) # on supprime le fichier généré pour nettoyer le script

        print("Nettoyage de la base de données terminée !")
...
if len(os.listdir(dataset_raw_path)) == 0:
...
async def preprocess(background_tasks: BackgroundTasks):
    ...
    return {"Status": "Preprocessing started. The training will be automatically launched. You'll get an e-mail when training is over.\
