from fastapi import FastAPI, HTTPException, Header, File, UploadFile, Depends, status
...
async def verify_token(token: str = Depends(oauth2_scheme)):
    ...
    print(f"Received token: {token}")  # Pour le débogage
    if token != "myfaketoken":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"Invalid JWT Token. Received: {token}"
        )
    return token

@app.get("/", summary="Page d'accueil")
def get_status(api_key: str = Depends(verify_api_key), token: str = Depends(verify_token)):
    response = {"status": "Service is running"}
    return response

@app.post("/predict", summary="Faire une prédiction")
async def get_prediction(file: UploadFile = File(...), api_key: str = Depends(verify_api_key), token: str = Depends(verify_token)):
    response = {"prediction": "Classe prédite", "score": "Score de prédiction"}
    return response

@app.post("/add_image", summary="Ajouter une image")
async def add_image(file: UploadFile = File(...), label: str = Header(None), api_key: str = Depends(verify_api_key), token: str = Depends(verify_token)):
    response = {"status": "Image ajoutée avec succès"}
    return response

@app.get("/get_species", summary="Obtenir la liste des espèces")
def get_species(api_key: str = Depends(verify_api_key), token: str = Depends(verify_token)):
