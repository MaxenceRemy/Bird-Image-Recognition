...
def download_dataset(dataset_name : str = "gpiosenka/100-bird-species", destination_folder : str = "./data", kaggle_json_path : str = "./kaggle.json"):
    """
    ...
    datasets = kaggle_api.dataset_list(search=dataset_name, sort_by='hottest') # Renvoie la liste des datasets qui conrrepondent aux critères de recherche
    ...
    if os.path.isfile(f"{destination_folder}/dataset_version.json"):
        with open(f"{destination_folder}/dataset_version.json", 'r') as file:
            ...
            if current_dataset_version >= online_dataset_version :
                logger.info(f"Le dataset est à jour, on ne le télécharge pas.")
                return
            else :
    ...
    for folder in folders:
        ...
        if os.path.exists(folder_to_delete) and os.path.isdir(folder_to_delete) : shutil.rmtree(folder_to_delete)

    # Fichiers
    files = ["birds.csv", "birds_backup.csv"]
    for file in files:
        file_to_delete = f"{destination_folder}/{file}"
        if os.path.exists(file_to_delete) and os.path.isfile(file_to_delete) : os.remove(file_to_delete)
    #endregion
    ...
    files_to_delete = glob.glob(os.path.join(destination_folder, 'EfficientNetB0*.h5')) # Fichiers correspondant au motif
