...
class DatasetCorrection:
    ...
    def download_full_dataset(self):
        ...
        response = requests.get(link) # Télechargement du fichier excel, répertoriant beaucoup d'espèces (11194 espèces au 31/07/24)
        with open(f"{self.db_to_clean}/birds_list.xlsx", "wb") as f:
            f.write(response.content)

        birds_list_df = pd.read_excel(f"{self.db_to_clean}/birds_list.xlsx")

        birds_list_df = birds_list_df[['IOC_14.1','English','French']] # On sélectionne les colonnes (dont les langues) à conserver
        birds_list_df = birds_list_df.rename(columns={"IOC_14.1": "BinomialNomenclature"})

        for column in birds_list_df.columns: # Pour chaque langue, on enlève les espaces parasites et on passe en minuscules sauf la premiere lettre.
            if column == "BinomialNomenclature" : continue
        ...
        birds_list_df.to_csv(f"{self.db_to_clean}/birds_list.csv", index = False) # Fichier csv répertoriant la liste d'espèces à proposer aux utilisateurs
    ...
    def test_phase_init(self):
        ...
        os.rename(f"{self.db_to_clean}/birds.csv", f"{self.db_to_clean}/birds_backup.csv") # Pour le test, on renomme le csv pour générer un backup
        with open(f"{self.db_to_clean}/birds_backup.csv", "r") as f_in, open(f"{self.db_to_clean}/birds.csv", "w") as f_out:
            first_line = True
            for row in f_in:
    ...
    def dataset_correction(self):
        """
        Correction du dataset (harmonisation des noms binominaux et vernaculaires anglais, correction de fautes, gestion des races, correction des chemins, création du nouveau csv)
        """

        logger.info(f"Correction du dataset et du fichier birds.csv")
        ...
        for species in species_to_delete:
            ...
            for folder in ["train", "test", "valid"]:
                ...
                if os.path.isdir(path) : shutil.rmtree(path) # Si le dossier existe, on le supprime 
        #endregion
        ...
        birds_list_df['BinomialNomenclature'] = birds_list_df['BinomialNomenclature'].astype(str).apply(lambda row: self.correctString(row))
        ...
        df['labels'] = df.apply(lambda row: binomial_to_english.get(row['scientific name'], row['labels']), axis=1)
        df['scientific name'] = df.apply(lambda row: english_to_binomial.get(row['labels'], row['scientific name']), axis=1)
    ...
    def correctString(self, string : str):
        """
        Renvoie la chaine de caractères sans espaces ni à droite ni à gauche, avec une majuscle au début en le reste en minuscule et sans apostroche
        """
    ...
    def full_correction(self):
        """
        Lance la correction complète (téléchargement du dataset, possible initialisation du dataset de test et correction)
        """

        if not os.path.isfile(f"{self.db_to_clean}/birds_list.csv") : self.download_full_dataset() # Si le fichier birds_list n'existe pas, on le télécharge
        if "ABBOTTS BABBLER" in os.listdir(f"{self.db_to_clean}/train") : self.dataset_correction() # Si le premier dossier est en majuscule, alors on effectue les corrections
        if self.test_mode : self.test_phase_init() # Si le mode test est activé, alors on réduit la taille du dataset 
