...
class TestPreprocessDataset(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.species_raw_folder = f"{cls.data_folder}/train/{cls.species.upper()}" # Avant preprocessing, les dossiers sont en majuscules
        cls.species_processed_folder = f"{cls.data_folder}/train/{cls.species.capitalize()}" # Après preprocessing, les dossiers sont en minuscules (sauf la premiere lettre)
    ...
    def test_01_main(self):
        ...
        self.assertFalse("Crimson sunbird" in os.listdir(f"{self.data_folder}/train"), f"Avant preprocessing, le répertoire {self.species_processed_folder} en minuscule ne devrait pas exister.")

        clean_db.cleanAll() # Étape de preprocessing

        # Le dossier 'CRIMSON BIRD' ne doit plus exister, mais 'Crimson sunbird' oui
        self.assertFalse("CRIMSON SUNBIRD" in os.listdir(f"{self.data_folder}/train"), f"Après preprocessing, le répertoire {self.species_raw_folder} en majuscule ne devrait pas exister.")
        self.assertTrue("Crimson sunbird" in os.listdir(f"{self.data_folder}/train"), f"Après preprocessing, le répertoire {self.species_processed_folder} en minuscule devrait exister.")

        # Test de l'existence du fichier birds_list.csv
        self.assertTrue(os.path.isfile(f"{self.data_folder}/birds_list.csv"), f"Le fichier {self.data_folder}/birds_list.csv n'existe pas.")
        ...
        with Image.open(f"{image_path}") as img:
            self.assertTrue(img.size == (224, 224), f"L'image {image_path} ne possède pas les dimensions attendues : {img.size} au lieu de (224, 224).")
