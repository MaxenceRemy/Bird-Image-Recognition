...
class TestDownloadDataset(unittest.TestCase):
    ...
    def test_01_main(self):
        ...
        Résultat attendu : Présence d'un dossier "train", d'un dossier "test", d'un dossier "valid", d'un fichier "birds.csv" et d'un fichier "dataset_version.json"
        """
        ...
        self.assertTrue(os.path.exists(self.train_folder), f"Le répertoire {self.train_folder} n'existe pas.") # Existence du dossier train
        self.assertTrue(os.path.exists(self.test_folder), f"Le répertoire {self.test_folder} n'existe pas.") # Existence du dossier test
        self.assertTrue(os.path.exists(self.valid_folder), f"Le répertoire {self.valid_folder} n'existe pas.") # Existence du dossier valid
        self.assertTrue(os.path.exists(self.species_folder), f"Le répertoire {self.species_folder} n'existe pas.") # Existence du dossier d'une espèce
        self.assertTrue(os.listdir(self.species_folder), f"Le répertoire {self.species_folder} est vide après le téléchargement.") # Dossier d'une espèce non vide
        self.assertTrue(os.path.isfile(self.dataset_version), f"Le fichier {self.dataset_version} n'existe pas.") # Existence du fichier de version du dataset
